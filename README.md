...
**Live:** https://symbols.konpo.co

## What this is

A logo takes hundreds of proposals. Only one gets picked. The rest get killed
...
Konpo Studio drew every symbol here as identity work for real clients. They
are finished proposals that lost their pitch rounds for the usual reasons (a
merger, a new CMO, "too modern"). Client details have been removed. A
portfolio only shows the winners; this is the rest of the work.

The catalogue keeps growing: new intakes are converted into the house
coordinate system and added as they arrive.

## The page

One self-contained HTML file (`symbol-index.html`, copied to `index.html` for
root serving; `npm run build` re-copies it). No framework. The single runtime dependency is lottie-web, self-hosted at
`/vendor/lottie.min.js`, for the animated marks; static marks are the
fallback, so nothing breaks without it.
UI sounds are synthesized live in WebAudio, with no audio files.
...
- Permalinks: every mark answers at `/#SYM-0XX`, `?sym=SYM-0XX`, and at
  `/s/sym-0xx`, its own page: the full shelter with that card open, carrying
  the mark's title, description, canonical and OG/Twitter tags (plus a
  noscript text fallback). Closing the card returns the address to `/`.
  Opening a mark rewrites the address to that path, so whatever anyone copies
  unfurls correctly;
  filtered/searched states use the query-param form instead
  (`?cat=…&q=…&view=…`), with pushState + back/forward support.
- OG images: `og/sym-0xx.png` (1200×630: the mark alone, centered on the
  tile color, no text) plus `og/root.png` (the Konpo mark) for the homepage.
  Image URLs carry `?v=` from each card's SVG so apps refetch changed cards.
...
- All of it is generated by `scripts/build-meta.mjs`. The mark pages are
  copies of `index.html`, so **deploy with `npm run deploy`**: it regenerates
  everything (`npm run build`) and then publishes, so no mark page serves an
  old copy. `vercel.json` sets `cleanUrls` so the extension-less paths resolve.
...
- Provenance: every rendered SVG embeds an ownership comment (© Konpo, the
  ...
  mark group (`k042`) that reads like styling but encodes the id, so lifted
